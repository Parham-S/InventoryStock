...
module.exports = {
  ...
  create: function create(context) {
    ...
    function isRangeTest(node) {
      ...
      function isBetweenTest() {
        if (node.operator === "&&" && astUtils.isSameReference(left.right, right.left)) {
          var leftLiteral = getNormalizedLiteral(left.left);
          var rightLiteral = getNormalizedLiteral(right.right);
      ...
      function isOutsideTest() {
        if (node.operator === "||" && astUtils.isSameReference(left.left, right.right)) {
          var leftLiteral = getNormalizedLiteral(left.right);
          var rightLiteral = getNormalizedLiteral(right.left);
    ...
    function getFlippedString(node) {
      ...
      var lastLeftToken = sourceCode.getTokenBefore(operatorToken);
      var firstRightToken = sourceCode.getTokenAfter(operatorToken);
      var source = sourceCode.getText();
      var leftText = source.slice(node.range[0], lastLeftToken.range[1]);
      var textBeforeOperator = source.slice(lastLeftToken.range[1], operatorToken.range[0]);
      var textAfterOperator = source.slice(operatorToken.range[1], firstRightToken.range[0]);
      var rightText = source.slice(firstRightToken.range[0], node.range[1]);
      var tokenBefore = sourceCode.getTokenBefore(node);
      var tokenAfter = sourceCode.getTokenAfter(node);
      var prefix = "";
      var suffix = "";
    ...
    return {
      BinaryExpression: function BinaryExpression(node) {
        ...
        var expectedNonLiteral = always ? node.right : node.left; // If `expectedLiteral` is not a literal, and `expectedNonLiteral` is a literal, raise an error.
