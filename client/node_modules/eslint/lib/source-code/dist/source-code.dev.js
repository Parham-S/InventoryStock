...
function _isSpaceBetween(sourceCode, first, second, checkInsideOfJSXText) {
  if (nodesOrTokensOverlap(first, second)) {
    return false;
  }

  var _ref = first.range[1] <= second.range[0] ? [first, second] : [second, first],
      _ref2 = _slicedToArray(_ref, 2),
      startingNodeOrToken = _ref2[0],
      endingNodeOrToken = _ref2[1];

  var firstToken = sourceCode.getLastToken(startingNodeOrToken) || startingNodeOrToken;
  var finalToken = sourceCode.getFirstToken(endingNodeOrToken) || endingNodeOrToken;
  var currentToken = firstToken;

  while (currentToken !== finalToken) {
    var nextToken = sourceCode.getTokenAfter(currentToken, {
     ...
     */
    checkInsideOfJSXText && nextToken !== finalToken && nextToken.type === "JSXText" && /[\t-\r \xA0\u1680\u2000-\u200A\u2028\u2029\u202F\u205F\u3000\uFEFF]/.test(nextToken.value)) {
      return true;
    }
...
function (_TokenStore) {
   ...
   */
  function SourceCode(textOrConfig, astIfNoConfig) {
    var _this;

    _classCallCheck(this, SourceCode);

    var text, ast, parserServices, scopeManager, visitorKeys; // Process overloading.

    if (typeof textOrConfig === "string") {
      text = textOrConfig;
      ast = astIfNoConfig;
    } else if (_typeof(textOrConfig) === "object" && textOrConfig !== null) {
    ...
    validate(ast);
    _this = _possibleConstructorReturn(this, _getPrototypeOf(SourceCode).call(this, ast.tokens, ast.comments));
    /**
    ...
    _this.visitorKeys = visitorKeys || Traverser.DEFAULT_VISITOR_KEYS; // Check the source text for the presence of a shebang since it is parsed as a standard line comment.

    var shebangMatched = _this.text.match(astUtils.shebangPattern);
    ...
    _this.lines.push(_this.text.slice(_this.lineStartIndices[_this.lineStartIndices.length - 1])); // Cache for comments found using getComments().
  ...
  _createClass(SourceCode, [{
    ...
    value: function getComments(node) {
      ...
      if (node.type === "Program") {
        ...
        var currentToken = this.getTokenBefore(node, {
          includeComments: true
        });
    ...
    value: function getJSDocComment(node) {
      ...
      var findJSDocComment = function findJSDocComment(astNode) {
        var tokenBefore = _this2.getTokenBefore(astNode, {
          includeComments: true
        });
    ...
    key: "isSpaceBetween",
    value: function isSpaceBetween(first, second) {
      return _isSpaceBetween(this, first, second, false);
    }
    /**
     * Determines if two nodes or tokens have at least one whitespace character
     * between them. Order does not matter. Returns false if the given nodes or
    ...
    value: function isSpaceBetweenTokens(first, second) {
      return _isSpaceBetween(this, first, second, true);
    }
    /**
    ...
    value: function getLocFromIndex(index) {
      ...
      var lineNumber = index >= this.lineStartIndices[this.lineStartIndices.length - 1] ? this.lineStartIndices.length : this.lineStartIndices.findIndex(function (el) {
        return index < el;
      });
      return {
        line: lineNumber,
    ...
    value: function getIndexFromLoc(loc) {
      if (_typeof(loc) !== "object" || typeof loc.line !== "number" || typeof loc.column !== "number") {
