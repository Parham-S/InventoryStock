...
module.exports = function (appPath, appName, verbose, originalDirectory, templateName) {
  ...
  if (!templateName) {
    ...
    console.error("You can fix this by running ".concat(chalk.cyan('npm uninstall -g create-react-app'), " or ").concat(chalk.cyan('yarn global remove create-react-app'), " before using ").concat(chalk.cyan('create-react-app'), " again."));
    return;
  }
  ...
  var templateJsonPath = path.join(templatePath, 'template.json');
  var templateJson = {};
  ...
  var templatePackage = templateJson["package"] || {}; // TODO: Deprecate support for root-level `dependencies` and `scripts` in v5.
  // These should now be set under the `package` key.

  if (templateJson.dependencies || templateJson.scripts) {
    console.log();
    console.log(chalk.yellow('Root-level `dependencies` and `scripts` keys in `template.json` are deprecated.\n' + 'This template should be updated to use the new `package` key.'));
  ...
  var templateScripts = templatePackage.scripts || {};
  ...
  var dependenciesToInstall = Object.entries(_objectSpread({}, templatePackage.dependencies, {}, templatePackage.devDependencies));

  if (dependenciesToInstall.length) {
    args = args.concat(dependenciesToInstall.map(function (_ref3) {
