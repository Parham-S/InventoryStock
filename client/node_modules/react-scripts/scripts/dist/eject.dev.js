...
var spawnSync = require('react-dev-utils/crossSpawn').sync;
...
function getGitStatus() {
  try {
    var stdout = execSync("git status --porcelain", {
...
prompts({
  ...
  function verifyAbsent(file) {
    if (fs.existsSync(path.join(appPath, file))) {
      console.error("`".concat(file, "` already exists in your app folder. We cannot ") + 'continue as you would lose all the changes in that file or directory. ' + 'Please move or delete it (maybe make a copy for backup) and run this ' + 'command again.');
      process.exit(1);
    }
  }

  var folders = ['config', 'config/jest', 'scripts']; // Make shallow array of files paths
  ...
  var jestConfig = createJestConfig(function (filePath) {
    return path.posix.join('<rootDir>', filePath);
  }, null, true);
  console.log();
  console.log(cyan("Copying files into ".concat(appPath)));
  folders.forEach(function (folder) {
    fs.mkdirSync(path.join(appPath, folder));
  });
  files.forEach(function (file) {
    var content = fs.readFileSync(file, 'utf8'); // Skip flagged files
    ...
    console.log("  Adding ".concat(cyan(file.replace(ownPath, '')), " to the project"));
  ...
  if (appPackage.devDependencies) {
    ...
    if (appPackage.devDependencies[ownPackageName]) {
      console.log("  Removing ".concat(cyan(ownPackageName), " from devDependencies"));
  ...
  if (appPackage.dependencies[ownPackageName]) {
    console.log("  Removing ".concat(cyan(ownPackageName), " from dependencies"));
  ...
  Object.keys(ownPackage.dependencies).forEach(function (key) {
    ...
    console.log("  Adding ".concat(cyan(key), " to dependencies"));
    appPackage.dependencies[key] = ownPackage.dependencies[key];
  }); // Sort the deps

  var unsortedDependencies = appPackage.dependencies;
  appPackage.dependencies = {};
  Object.keys(unsortedDependencies).sort().forEach(function (key) {
  ...
  Object.keys(appPackage.scripts).forEach(function (key) {
    Object.keys(ownPackage.bin).forEach(function (binKey) {
      ...
      console.log("  Replacing ".concat(cyan("\"".concat(binKey, " ").concat(key, "\"")), " with ").concat(cyan("\"node scripts/".concat(key, ".js\""))));
    });
  });
  console.log();
  console.log(cyan('Configuring package.json')); // Add Jest config

  console.log("  Adding ".concat(cyan('Jest'), " configuration"));
  appPackage.jest = jestConfig; // Add Babel config

  console.log("  Adding ".concat(cyan('Babel'), " preset"));
  appPackage.babel = {
    presets: ['react-app']
  }; // Add ESlint config

  if (!appPackage.eslintConfig) {
    console.log("  Adding ".concat(cyan('ESLint'), " configuration"));
  ...
  if (fs.existsSync(paths.appTypeDeclarations)) {
    try {
      ...
      var content = fs.readFileSync(paths.appTypeDeclarations, 'utf8');
      var ownContent = fs.readFileSync(paths.ownTypeDeclarations, 'utf8').trim() + os.EOL; // Remove react-scripts reference since they're getting a copy of the types in their project
  ...
  if (fs.existsSync(paths.yarnLockFile)) {
    var windowsCmdFilePath = path.join(appPath, 'node_modules', '.bin', 'react-scripts.cmd');
    var windowsCmdFileContent;
