...
function guessEditor() {
  ...
  try {
    if (process.platform === 'darwin') {
      ...
      var _output = child_process.execSync('wmic process where "executablepath is not null" get executablepath').toString();

      var runningProcesses = _output.split('\r\n');

      for (var _i = 0; _i < runningProcesses.length; _i++) {
        var processPath = runningProcesses[_i].trim();

        var _processName = path.basename(processPath);

        if (COMMON_EDITORS_WIN.indexOf(_processName) !== -1) {
          return [processPath];
        }
      }
      ...
      var _output2 = child_process.execSync('ps x --no-heading -o comm --sort=comm').toString();

      var _processNames = Object.keys(COMMON_EDITORS_LINUX);

      for (var _i2 = 0; _i2 < _processNames.length; _i2++) {
        var _processName2 = _processNames[_i2];

        if (_output2.indexOf(_processName2) !== -1) {
          return [COMMON_EDITORS_LINUX[_processName2]];
        }
      }
    }
  } catch (error) {} // Ignore...
