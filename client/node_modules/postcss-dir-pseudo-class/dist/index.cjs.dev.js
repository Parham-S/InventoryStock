...
var index = postcss.plugin('postcss-dir-pseudo-class', function (opts) {
  ...
  return function (root) {
    ...
    root.walkRules(/:dir\([^\)]*\)/, function (rule) {
      var currentRule = rule; // conditionally preserve the original rule
        ...
        selectors.nodes.forEach(function (selector) {
          ...
          selector.walk(function (node) {
            if ('pseudo' === node.type && ':dir' === node.value) {
              ...
              var nextIsSpaceCombinator = next && next.type && 'combinator' === next.type && ' ' === next.value; // preserve the selector tree
              ...
              var value = node.nodes.toString(); // whether :dir matches the presumed direction

              var isdir = dir === value; // [dir] attribute
