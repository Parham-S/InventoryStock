...
function getSocketUrlParts(resourceQuery) {
  ...
  var urlParts = url.parse(scriptSource);
  /** @type {string | undefined} */

  var auth;
  var hostname = urlParts.hostname;
  var protocol = urlParts.protocol;
  var pathname = '/sockjs-node'; // This is hard-coded in WDS

  var port = urlParts.port; // FIXME:
  ...
  var authUrlParts = new URL(scriptSource, 'http://foo.bar'); // Parse authentication credentials in case we need them
  ...
  var isEmptyHostname = urlParts.hostname === '0.0.0.0' || urlParts.hostname === '::' || urlParts.hostname === null; // We only re-assign the hostname if we are using HTTP/HTTPS protocols
  ...
  var parsedQuery = parseQuery(resourceQuery || '');
